...
class ManagerTestCase(TestCase):
    def test_user_order_by(self):
        ...
        for field_name in field_names:
            ...
            asc_ids = [obj.item.id for obj in order_model.objects.all().\
                    order_by(field_name)]
            for i, obj in enumerate(asc_objects):
                self.failUnless(obj.id == asc_ids[i])

            # Descedning is achieved by adding '-' to field name.
            desc_objects = ordered_model.objects.all().user_order_by('-%s' % \
                    field_name)
            desc_ids = [obj.item.id for obj in order_model.objects.all().\
                    order_by('-%s' % field_name)]
...
class ModelsTestCase(TestCase):
    def test_model_creation(self):
        ...
        """
        orderable_models = [key.split('.')[-1] for key in \
                settings.ORDERABLE_MODELS.keys()]
        for model in get_models(models):
            model_name = model._meta.object_name.replace('OrderItem', '')
            self.failUnless(model_name in orderable_models, 'OrderItem model \
                    for %s not created.' % model_name)
